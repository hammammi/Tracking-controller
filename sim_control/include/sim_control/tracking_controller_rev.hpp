...
class tracking_controller{
    ...
    void subscriber_declaration(){
        // Subscribe
        subscriber_state = nh_.subscribe("/camera/odom/sample",1,&tracking_controller::callback_state,this);
    ...
    void cmd_vel_pub(){
        ...
        // Acceleration commend : velocity error
        ax_cmd   += K_v[0]*(vx_des - vx_robot);
        ay_cmd   += K_v[1]*(vy_des - vy_robot);
        aphi_cmd += K_v[2]*(vphi_des - vphi_robot);
        ...
        // Control Input
        vx_cmd = ax_cmd*dt + K_p[0] * x_err;
        vy_cmd = ay_cmd*dt + K_p[1] * y_err;
        vphi_cmd = aphi_cmd*dt + K_p[2] * phi_err;
        ...
        if(sqrt((x_goal-x_robot)*(x_goal-x_robot)+(y_goal-y_robot)*(y_goal-y_robot))<0.005 && fabs(phi_robot[1]-phi_goal)<0.008){
            cmd_motor_vel.w[0] = 0;
            cmd_motor_vel.w[1] = 0;
            cmd_motor_vel.w[2] = 0;
            cmd_motor_vel.w[3] = 0;
    ...
    void publish_slam_pose(){
        ...
        // Get SLAM position
        slam_pose.pose.pose.position.x = Transform.getOrigin().x();
        slam_pose.pose.pose.position.y = Transform.getOrigin().y();
